...
| First-harmonic and large-prime pivot reductions in the companion | The additive-character step and the factorisation \(\varphi(mp)=\varphi(m)(p-1)\) use standard finite Fourier and multiplicative-number-theory ingredients; Apostol supplies the cited arithmetic background. The release's exact contribution is the four-term supplier-fibre decomposition, its one-sided \(14/25,1/100,1/100,8/25\) budget, and its composition with certificate completeness. | No matching combined theorem is recorded, but this is not a novelty assertion. The decomposition is an exact conditional consumer: no prime-distribution or residual-decorrelation estimate populating the four budgets is proved. |
