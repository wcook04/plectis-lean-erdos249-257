...
This is a self-contained public Lean project and a Plectis case study. It shows
the kind of source-linked mathematical artefact that Plectis is intended to
support. It is not an entrypoint into any private development system, and no
equivalence with a private source is asserted. In particular, do not infer results from private or unreleased work.
...
This project formalises results around both questions. **It does not solve
Erdős #249 or the universal form of Erdős #257.**
...
| **verified finite instance** | The kernel checks 28 explicit lcm-diagonal scales through `t = 64`. A finite range does not change the unbounded quantifier above. |
| **proved here** | For the #257 test value `1/2`, achievement-set membership is equivalent to infinitely many greedy skips. If there is a last skip, the remaining branch is an exact tail estimate for two middle coordinates. An isolated unsafe skip is confined to a narrow two-thirds band, but this does not show that the actual orbit avoids the band. |
